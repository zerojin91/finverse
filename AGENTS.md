...
- 모의투자 설정 흐름은 `종목 선택 → 자료 수집 → 투자 상태 → 시뮬레이션 설정` 순서다. 첫 화면에는 종목 선택만 보이고, 종목 선택 완료 시 자료 수집, 자료 준비 완료 시 투자 상태, 필수 투자값 입력 완료 시 시뮬레이션 설정과 시작 버튼을 별도 확인 버튼 없이 자동으로 순차 공개한다. 각 단계가 열리면 해당 섹션을 모달 화면 중앙으로 부드럽게 스크롤한다. 신규 투자는 실제 투자 가능 금액을 받고, 기존 보유는 투자 금액 영역을 숨긴 채 평단·수량만 받아 추가 현금 0원으로 시작한다. 사용자는 10·20·60거래일 기간과 균형·위기 대응·기회 포착·무작위 연습 유형을 선택한다.
