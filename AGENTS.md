...
- 게임 화면의 우측 패널은 `투자 시뮬레이션`으로 표시한다. 평소에는 World Agent의 환경 갱신과 59명 반응을 관찰하고, 중요한 사건일에만 사용자 판단·주문·근거·확신도를 기록하게 안내한다. 에이전트 반응 로그는 전체 폭에서 캔들 형성과의 연결을 읽을 수 있게 표시한다.
...
- 각 거래일의 59개 체결이 끝나면 네 수급 주체별 매수·매도·관망 집계, 가격 구성요소, 공개 환경·사건을 OpenRouter에 전달해 `market_summary_detail`을 생성하고 `agent_rounds`에 함께 저장한다. 우측 패널은 이 결과의 전체 요약·주체별 행동·`주가 변동 이유 추론`을 보여준다. LLM 호출이 실패해도 규칙 기반 집계 요약으로 거래를 계속하며, 예측이 아닌 교육용 사후 해설로 취급한다.
