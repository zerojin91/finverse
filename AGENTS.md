...
- 프론트엔드·백엔드·게이트웨이 등 실행 중인 서비스에 반영되는 코드·설정을 변경한 뒤에는 테스트 실행 여부와 무관하게 항상 해당 서비스를 재시작한다. 문서만 수정한 경우에는 재시작하지 않는다.
- 사용자가 UI에서 직접 확인하겠다고 명시하면, 빌드·자동 테스트·브라우저 확인은 사용자가 별도로 요청할 때만 실행한다. 이 경우에도 실행 중인 서비스에 반영되는 변경이라면 서비스 재시작은 반드시 수행하고, 수정 범위와 로컬 커밋을 알린 뒤 사용자의 UI 확인을 기다린다.
