...
- 모의투자 설정 흐름은 `종목 선택 → 자료 수집 → 투자 상태 → 시뮬레이션 설정` 순서다. 신규 투자는 실제 투자 가능 금액을 받고, 기존 보유는 투자 금액 영역을 숨긴 채 평단·수량만 받아 추가 현금 0원으로 시작한다. 사용자는 10·20·60거래일 기간과 균형·위기 대응·기회 포착·무작위 연습 유형을 선택한다.
