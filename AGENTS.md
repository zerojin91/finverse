...
- 단순 문구·스타일·간격처럼 기존 동작 경로와 데이터 연결을 바꾸지 않는 작은 UI 수정은 정적 확인과 빌드 오류 확인까지만 하고, 실제 화면 확인은 사용자에게 먼저 요청한다. 불필요한 전체 테스트·서비스 재시작을 기본으로 하지 않는다.
- API 계약·데이터 조회·DB 연결·캐시·비동기 작업·포트/프로세스처럼 반복 장애 이력이 있거나 동작 경로가 바뀌는 수정은 관련 단위 테스트와 직접 API 점검을 반드시 수행한다. 필요한 서비스 재시작도 이 경우에만 한다.
- 사용자가 UI에서 직접 확인하겠다고 명시하면, 빌드·자동 테스트·서비스 재시작·브라우저 확인은 사용자가 별도로 요청할 때만 실행한다. 이 경우에는 수정 범위와 로컬 커밋만 알리고 사용자의 UI 확인을 기다린다.
