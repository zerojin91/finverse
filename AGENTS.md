...
- 평상시 거래일에는 실제 주문 대신 `내일 매수 고려·관찰 계속·내일 매도 고려` 중 하나를 일일 판단으로 기록한다. 이 기록은 시장 체결이나 가격에 영향을 주지 않으며, 날짜별 시장 요약과 함께 저장돼 최종 회고에서 다음 거래일 결과와 비교한다. 진행 UI는 `하루씩 보기`와 `중요 사건 전까지 빠르게 진행`을 제공하며, 빠른 진행도 중요한 사건의 사용자 판단 게이트는 건너뛰지 않는다.
