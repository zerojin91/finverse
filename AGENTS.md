...
- 사용자 체결 원장에는 체결이 실제 반영된 거래일(`market_date`)을 반드시 저장한다. 체결내역 UI와 보고서는 주문을 누른 시각이 아니라 이 값을 사용한다. 수익률은 `현재 현금 + 현재가×보유수량`인 순자산을 `initial_equity`와 비교하며, 매수·매도 수수료와 세금은 현금·실현손익에 반영한다. 체결 사례를 수정할 때는 날짜 누락과 수익률을 함께 검증한다.
