...
- 조치: `npm run db:tunnel`을 별도 장기 세션으로 재실행하고, 15432 수신 상태와 5055/3000 캔들 API의 실제 응답을 확인했다.
- 재발 방지: DB 오류의 첫 점검은 항상 15432 포트 수신 여부다. 터널이 없으면 DB·프론트 코드를 수정하지 않고 터널부터 복구하며, `npm run dev`와 터널을 독립적으로 관리한다.
