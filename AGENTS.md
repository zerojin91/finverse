...
- 게임 화면의 우측 패널은 `투자 시뮬레이션`으로 표시한다. 평소와 중요한 사건 모두 `내일 매수 고려·관찰 계속·내일 매도 고려` 중 하나만 선택하게 하며, World 모드에서는 수량·주문·판단 근거·확신도를 입력하지 않는다. 사용자의 선택은 시장 가격·수급에 영향을 주지 않는 학습 기록이다. 에이전트 반응 로그는 전체 폭에서 캔들 형성과의 연결을 읽을 수 있게 표시한다.
- 평상시 거래일에는 실제 주문 대신 `내일 매수 고려·관찰 계속·내일 매도 고려` 중 하나를 일일 판단으로 기록한다. 선택하지 않아도 `관찰 계속`을 기본 판단으로 거래일 원장에 저장하고, 사용자가 나중에 선택하면 같은 날짜 기록을 사용자 판단으로 갱신한다. 이 기록은 시장 체결이나 가격에 영향을 주지 않으며, 날짜별 시장 요약과 함께 저장돼 최종 회고에서 다음 거래일 결과와 비교한다. 진행 UI는 `자동 진행`과 `하루 진행`을 제공하며, 자동 진행도 중요한 사건의 사용자 판단 게이트는 건너뛰지 않는다.
...
- 조치: 바의 너비를 `current_day_index / simulation_days`로 고정해 실제 미래 거래일이 완료된 뒤에만 채워지도록 수정했다. 작업 중 에이전트 처리 상태는 바의 애니메이션만 사용한다.
