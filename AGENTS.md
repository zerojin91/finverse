...
- 로그인 계정은 첫 모의투자 전에 5문항 `현재 나의 투자 성향` 진단을 한 번 완료해야 한다. 진단 기록이 없을 때 `판단 해보기`를 누르면 설정 화면을 닫거나 투자 일지로 이동하지 않고 같은 모달에서 설문을 진행한다. 저장 성공 후 성향 결과를 보여주고, 사용자가 `시뮬레이션 시작`을 눌러 같은 모달의 설정으로 이어진다. 결과(점수·완료 시각)는 로컬 인증 SQLite의 `investor_profiles`에 계정별로 저장하고, 재진단 시 같은 행을 갱신한다. 이전 브라우저 로컬 저장 결과는 다른 계정으로 절대 이전하거나 재사용하지 않는다. `GET/POST /api/investor-profile`은 세션 사용자만 읽고 쓸 수 있으며, Next 모의투자 프록시는 신규 `POST /api/paper-trading/scenarios` 전에 이 기록을 반드시 확인해 없으면 403으로 거부한다. 화면 안내만으로 이 제약을 대체하지 않는다.
...
- 시뮬레이션은 상태 유지형 World Agent와 59개 투자 에이전트의 폐루프다. 거래일 안에서는 `환경 갱신 → 공개 정보/사건 → 개별 에이전트 행동 → 주문 체결·시장 결과 저장 → 사용자 일일 판단 기록` 순서를 지킨다. 중요한 사건도 독립 단계로 멈추지 않고, 같은 거래일의 시장 반응·캔들·요약과 함께 사용자에게 공개한 뒤 매수·매도·관망 판단을 받는다. 사용자의 판단은 이미 형성된 당일 시장 가격·수급을 바꾸지 않으며, 매수·매도 고려만 다음 거래일 개인 계좌 예약 주문으로 반영된다. 에이전트 주문 결과만 다음 거래일 WorldState의 가격·수급·유동성·변동성·심리에 피드백된다. 에이전트는 외생 사건의 사실·내용·발생 자체를 바꾸지 못한다.
...
- 게임 화면의 차트는 실제 이력을 항상 최근 10거래일만 보이고, 선택한 10·20·60거래일 전체 폭은 미래 슬롯으로 미리 확보한다. 실제 이력도 양봉·음봉 색을 유지하되 투명도로 현재 시뮬레이션과 구분하며, 진행된 거래일만 오른쪽 슬롯을 채운다. 차트 폭이 패널보다 넓으면 새 거래일이 완료될 때마다 최신 시뮬레이션 캔들을 따라 한 슬롯씩 자동 이동해 진행된 봉이 화면 밖에 고정되지 않게 한다.
...
- 평상시 거래일에는 `내일 매수 고려·관찰 계속·내일 매도 고려` 중 하나를 일일 판단으로 기록한다. 선택하지 않아도 `관찰 계속`을 기본 판단으로 거래일 원장에 저장하고, 사용자가 매수·매도 고려를 고르면 1주 이상 수량을 입력한 뒤 `판단 기록`을 명시적으로 눌러야 다음 거래일 시장 결과 이후 개인 계좌에만 체결한다. 수량 입력란의 포커스 이탈만으로 주문을 기록하지 않는다. 매수는 현금 한도, 매도는 보유 수량 한도를 검증하며, 체결 내역·잔고·평가액·손익은 `portfolio`와 `fills`에 보존한다. 이 기록과 사용자 체결은 시장 체결이나 가격에 영향을 주지 않으며 최종 회고에서 다음 거래일 결과와 비교한다. 진행 UI는 `다음 거래일 진행`만 제공하며, 중요한 사건의 사용자 판단 게이트는 건너뛰지 않는다.
...
#### 2026-09-07 — Tailscale 직접 경로 단절 후 SSH DB 터널 복구

- 증상: 종목 선택에서 `finverse PostgreSQL 연결에 실패했습니다: connection timeout expired`가 표시되고 5055 캔들 API가 503을 반환했다.
- 원인: 현재 PC의 Tailscale 피어 목록에 DB 서버 `100.89.226.42`가 없어 직접 PostgreSQL 경로가 닿지 않았다. 원격 수집 서버의 `finverse-db` 컨테이너와 `100.89.226.42:5432` 수신은 정상임을 SSH로 확인했다.
- 조치: SSH DB 터널을 `127.0.0.1:15432`에 재실행하고, 직접 연결 모드로 남아 있던 3000·5055 서비스만 터널 모드로 재시작했다. 5055 캔들 API가 200과 실제 일봉을 반환하는 것을 확인했다.
- 재발 방지: Tailscale 직접 접속이 되지 않을 때는 DB 주소를 임의로 변경하지 않고, SSH로 원격 DB 컨테이너·5432 수신을 먼저 확인한 뒤 표준 15432 터널을 복구한다.
...
#### 2026-09-07 — 터널 복구 뒤 모의투자 API가 이전 직접 DB 주소를 유지

- 증상: 로컬 `15432` SSH 터널과 터널 경유 SQL 조회는 성공했지만, 종목 선택 화면은 `finverse PostgreSQL 연결에 실패했습니다: connection timeout expired`를 반환했다.
- 원인: 이전 설정으로 실행된 5055 Flask 프로세스가 여러 개 겹쳐 있었고, 직접 Tailscale DB 주소를 이미 읽은 프로세스가 요청을 처리했다.
- 조치: 기존 5055 프로세스를 정리하고 프로젝트 `.venv`의 Python으로 `-m services.paper_trading_api`를 재기동해 터널 설정을 다시 읽게 했다. 3000 프록시의 종목 캔들 API가 200으로 복구된 것을 확인했다.
- 재발 방지: 터널 상태만 확인하지 말고 터널 경유 SQL과 5055 직접·3000 프록시 캔들 API를 함께 점검한다. Flask 재기동 시 전역 Python이 아닌 프로젝트 `.venv`를 사용한다.
