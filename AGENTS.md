...
- 로그인 계정은 첫 모의투자 전에 5문항 `현재 나의 투자 성향` 진단을 한 번 완료해야 한다. 결과(점수·완료 시각)는 로컬 인증 SQLite의 `investor_profiles`에 계정별로 저장하고, 재진단 시 같은 행을 갱신한다. `GET/POST /api/investor-profile`은 세션 사용자만 읽고 쓸 수 있으며, Next 모의투자 프록시는 신규 `POST /api/paper-trading/scenarios` 전에 이 기록을 반드시 확인해 없으면 403으로 거부한다. 화면 안내만으로 이 제약을 대체하지 않는다.
