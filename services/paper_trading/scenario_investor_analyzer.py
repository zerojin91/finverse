...
def analyze_scenario_investor(game: dict[str, Any]) -> dict[str, Any]:
    ...
    event_reflections = [row for row in daily_reflections if row.get("event_id")]
    ...
    high_confidence = [fill for fill in fills
                       if float(fill.get("confidence") or 0) >= 80]
    loss_avoidance_sells = [fill for fill in fills
    ...
    if event_reflections:
        findings.append(f"중요 사건에서 방향성 판단을 {len(event_reflections)}회 기록했습니다.")
        lessons.append({"topic": "사건 판단 기록", "message": "사건의 첫 인상과 실제 시장 반응을 분리해 기록하고, 다음 거래일 결과와 비교해 보세요."})
    ...
    if game.get("mode") == "world":
        if not daily_reflections:
            findings.append("아직 기록된 사용자 판단이 없습니다.")
    elif not fills:
    ...
    if game.get("mode") == "world":
        metrics["world_decision_count"] = len(event_reflections)
    return {
        "style": style,
        "metrics": metrics,
