...
def generate_llm_reports(game: dict[str, Any],
    ...
    scenario_message = {"role": "user", "content": (
        ...
        "단, 보고서 본문에는 retail_001 같은 개별 에이전트 ID, 번호 범위, 개별 주문 수량·금액을 절대 나열하지 마라. "
        "각 주체별로 관망·위험 축소·저가 매수·추세 추종·헤지 등 행동 유형이 어떤 비중과 흐름을 보였는지, "
        "어떤 시장 신호 또는 이벤트에 반응해 전환됐는지, 그 반응이 가격·수급 변화와 어떻게 이어졌는지를 자연스러운 줄글로 설명하라. "
        "검증된 집계 수치가 필요하면 주체별 합계 또는 비율만 제한적으로 사용하라. "
