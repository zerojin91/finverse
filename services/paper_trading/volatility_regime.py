...
CLUSTER_MEMORY = .85
# 한 종목의 변동성이 평시 대비 오갈 수 있는 범위. 너무 넓으면 큰 하루가 레벨을
# 밀어올리고 그 레벨이 다음 하루를 더 키우는 상승 나선이 돌아, 전체 변동성이
# 이력의 159%까지 뜨고 그 소음에 이벤트가 묻힌다(실측). 너무 좁으면 조용한
# 구간이 계속 조용하다. 4000세션 기준 이 값이 변동성 107%, 이벤트 2.1배다.
CLUSTER_BOUNDS = (.7, 2.0)
