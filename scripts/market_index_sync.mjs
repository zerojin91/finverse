...
const localDatabase = Boolean(process.env.POSTGRES_PASSWORD);
const host = localDatabase ? process.env.POSTGRES_HOST || "127.0.0.1" : process.env.FINVERSE_DB_WRITE_HOST || process.env.FINVERSE_DB_HOST;
const database = localDatabase ? process.env.POSTGRES_DB || "finverse" : process.env.FINVERSE_DB_WRITE_NAME || process.env.FINVERSE_DB_NAME;
const username = localDatabase ? process.env.POSTGRES_USER || "finverse" : process.env.FINVERSE_DB_WRITE_USER || process.env.FINVERSE_DB_USER;
const password = localDatabase ? process.env.POSTGRES_PASSWORD : process.env.FINVERSE_DB_WRITE_PASSWORD || process.env.FINVERSE_DB_PASSWORD;
if (!host || !database || !username || !password) throw new Error("FINVERSE DB environment is missing");
...
const sql = postgres({ host, port: Number(process.env.FINVERSE_DB_PORT || 5432), database, username, password, max: 1, connect_timeout: 10, prepare: false, ssl: false });
try {
  if (rows.length) await sql`
    insert into lake.records ${sql(rows, "record_id", "collector", "record_type", "source", "schema_version", "record_hash", "collected_at", "payload")}
